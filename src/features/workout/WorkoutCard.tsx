...
type Props = {
  ...
  durationMin: number; kcal: number; avgBpm: number;
  onStart?: () => void;
...
export default function WorkoutCard(p: Props) {
  ...
  const Label = ({ icon, text }: { icon: string; text: string }) => (
    <View style={{ flexDirection: "row", alignItems: "center" }}>
      <Icon name={icon} />
      <Text style={{ fontSize: 14, fontWeight: "600", color: c.textMuted, marginLeft: 6 }}>
        {text}
      </Text>
    </View>
  );
    ...
    <Card variant="elevated" style={{ gap: 8 }}>
      ...
      <View style={styles.headerRow}>
        <View style={styles.headerLeft}>
          <View
            style={{
              width: 36, height: 36, borderRadius: d.radii.md,
              alignItems: "center", justifyContent: "center",
              backgroundColor: d.theme.scheme === "dark" ? "#3a2a00" : "#fff2cc",
            }}
          >
            <Text style={{ fontSize: 20, color: c.iconSoft }}>🏋️</Text>
          </View>
          <View style={{ flex: 1, paddingRight: 12 }}>
            <Text style={[styles.title, { color: c.textStrong }]} numberOfLines={1}>{p.title}</Text>
            <Text style={[styles.subtitle, { color: c.textMuted }]} numberOfLines={1}>{p.subtitle}</Text>
          </View>
        </View>
      </View>
