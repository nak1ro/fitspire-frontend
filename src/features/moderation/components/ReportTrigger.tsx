    ...
    return (
        <>
            <button onClick={() => setOpen(true)} className={compact
                ? 'rounded-lg p-1.5 text-surface-400 transition-colors hover:bg-surface-100 hover:text-error'
                : 'inline-flex items-center gap-1.5 text-[11px] font-semibold text-surface-500 transition-colors hover:text-error'} aria-label={`Report ${target.label}`}>
